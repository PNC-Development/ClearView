<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="organizations.aspx.cs" Inherits="NCC.ClearView.Presentation.Web.organizations" %>
...
<script type="text/javascript">
    ...
    var oDisc = null;
    var oEnabled = null;
    function Edit(strId, strName, strCode, strUser, strUserId, strDisc, strEnabled) {
    ...
    function Load() {
        ...
        oDisc = document.getElementById('<%=chkDisc.ClientID%>');
		    ...
		    <td colspan="2" align="center">
                        ...
                        <tr>
                            <td>
                                <asp:repeater ID="rptView" runat="server">
                                    ...
                                    <ItemTemplate>
                                        <tr bgcolor="#EFEFEF" onmouseover="AdminMouseOver(this);" onmouseout="AdminMouseOut(this);" onclick="if(event.srcElement.tagName != 'INPUT')Edit('<%# DataBinder.Eval(Container.DataItem, "organizationid") %>','<%# DataBinder.Eval(Container.DataItem, "name") %>','<%# DataBinder.Eval(Container.DataItem, "code") %>','<%# oUser.GetFullName(Int32.Parse(DataBinder.Eval(Container.DataItem, "userid").ToString())) %>','<%# DataBinder.Eval(Container.DataItem, "userid") %>','<%# DataBinder.Eval(Container.DataItem, "nodisc") %>','<%# DataBinder.Eval(Container.DataItem, "enabled") %>');" class="default">
                                            <td width="5%" align="left">&nbsp;<asp:ImageButton ID="btnDelete" OnClick="btnDeleteLink_Click" runat="server" ToolTip="Delete" ImageAlign="AbsMiddle" ImageUrl="/admin/images/delete.gif" CommandArgument='<%# DataBinder.Eval(Container.DataItem, "organizationid") %>' /></td>
                                            <td width="50%">&nbsp;<%# DataBinder.Eval(Container.DataItem, "name") %></td>
                                            <td width="50%">&nbsp;<%# DataBinder.Eval(Container.DataItem, "modified") %></td>
                                            <td align="center"><asp:ImageButton ID="btnEnable" runat="server" ImageAlign="AbsMiddle" ImageUrl='<%# (DataBinder.Eval(Container.DataItem, "enabled").ToString() == "1") ? "/admin/images/enabled.gif" : "/admin/images/disabled.gif" %>' CommandArgument='<%# DataBinder.Eval(Container.DataItem, "organizationid") %>' OnClick="btnEnable_Click" /></td>
                                        </tr>
                                    </ItemTemplate>
                                    <FooterTemplate>
                                        </table>
                ...
                <div id="divAdd" runat="server" style="display:none">
                    <table width="95%" cellpadding="3" cellspacing="0" border="0" align="center">
                        ...
                        <tr> 
                            <td class="default">Allow Empty DISC:</td>
                            <td><asp:CheckBox ID="chkDisc" runat="server" Checked="true" /></td>
                        </tr>
                        <tr> 
